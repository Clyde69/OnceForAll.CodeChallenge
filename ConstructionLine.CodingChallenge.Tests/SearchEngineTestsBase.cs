...
namespace ConstructionLine.CodingChallenge.Tests
{
    public class SearchEngineTestsBase
    {
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            var expectedResults = shirts.Where(w =>
                (!searchOptions.Sizes.Any() || searchOptions.Sizes.Contains(w.Size))
                &&
                (!searchOptions.Colors.Any() || searchOptions.Colors.Contains(w.Color)));

            foreach (var size in Size.All)
            {
                var sizeCount = sizeCounts.SingleOrDefault(s => s.Size.Id == size.Id);
                Assert.That(sizeCount, Is.Not.Null, $"Color count for '{size.Name}' not found in results");

                var expectedSizeCount = expectedResults.Count(w => w.Size.Id == size.Id);

                Assert.That(sizeCount.Count, Is.EqualTo(expectedSizeCount),
                    $"Color count for '{sizeCount.Size.Name}' showing '{sizeCount.Count}' should be '{expectedSizeCount}'");
            }
        }
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = expectedResults.Count(w => w.Color.Id == color.Id);
